...
function RouteRow({ route, isBest, onClick, isSelected }) {
  ...
  const rowStyle = isSelected ? { background: '#1e3a5f' } : {};

  return (
    <tr style={{ ...rowStyle, cursor: 'pointer' }} onClick={() => onClick(route)}>
      <td style={tdStyle}>
        {route.exit}
...
export default function ResultsTable({ result, onSelectPath }) {
  ...
  const tabs = [
    { key: 'primary', label: `${result.primary_routes[0]?.algorithm?.toUpperCase() || 'Primary'} Routes` },
        ...
        <table style={s.table}>
          ...
          <tbody>
            {primaryRoutes.map((r, i) => (
              <RouteRow key={r.exit} route={r} isBest={i === 0 && r.reachable}
