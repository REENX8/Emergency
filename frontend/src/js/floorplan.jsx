...
const NODE_R = { room: 24, corridor: 18, stair: 16, exit: 16 };
...
function FloorPlan({
  ...
  /* Intensity per node from fireSpread + current time */
  const fireIntensity = React.useMemo(() => {
  ...
  const compareSet = React.useMemo(() => {
    const s = new Set();
  ...
  const bottleneckSet = React.useMemo(() => {
    const s = new Set();
    bottleneckEdges.forEach(([u, v]) => { s.add(`${u}__${v}`); s.add(`${v}__${u}`); });
    return s;
  }, [bottleneckEdges]);

  const nodeOnPath = React.useMemo(() => {
    ...
    <svg viewBox={`0 0 ${W} ${H}`} className="floorplan" preserveAspectRatio="xMidYMid meet">
      ...
      <g>
        {edges.map(e => {
          ...
          let dash = '';
          if (s > 0.05 && !blocked) {
            const hueShift = s; // 0 → grey, 1 → red
            stroke = `oklch(${0.62 - hueShift * 0.15} ${0.06 + hueShift * 0.16} ${50 - hueShift * 30})`;
            strokeWidth = 2 + s * 2;
          }
          if (blocked) {
      ...
      {nodes.map(n => {
        ...
        if (intensity != null) {
          /* fire intensity mapping yellow → deep red */
        ...
        const shape = n.type === 'room' ? 'rect' : 'rect';
        ...
        return (
          <g key={n.id} className="node"
            ...
            <text x={n.x} y={n.y + 11} className="node-cap" textAnchor="middle">
              {n.type === 'room' ? `${n.cap}p` : n.type === 'exit' ? 'EXIT' : n.type === 'stair' ? '↕' : '═'}
            </text>
            {/* Max-flow badge */}
            {isMax && (
              <g>
