...
const edgeWeight = (e, smoke = 0, density = 0) => {
...
const propagateSmoke = (nodes, edges, fireNodeId, windDeg = 135) => {
  if (!fireNodeId) return {};
  const fire = nodes.find(n => n.id === fireNodeId);
  if (!fire) return {};
  const out = {};
  const lambda = 0.015;            // tuned for pixel-space distance
  ...
  edges.forEach(e => {
    ...
    /* Cross-floor smoke is heavily attenuated */
    if (u.floor !== fire.floor && v.floor !== fire.floor) dist *= 2.6;
    /* edge direction vs wind */
...
const dijkstra = (nodes, edges, adj, source, smoke, density) => {
  const dist = {}, prev = {};
...
const planEvacuation = (nodes, edges, opts) => {
  const {
    fire_location, occupied_rooms = [], blocked_exits = [],
...
const computeFireSpread = (nodes, edges, fireNodeId, windDeg = 135, windSpeed = 3.5) => {
...
const safetyScore = (nodes, edges) => {
  const V = nodes.length;
  const exits = nodes.filter(n => n.type === 'exit').length;
  const nonExitCap = nodes.filter(n => n.type !== 'exit').reduce((s, n) => s + n.cap, 0);
  const minCut = 2;                             // stub for prototype
  const edgeConn = 1;                           // stub
  const k = 1;
  const s1 = 40 * Math.min(1, exits / (V / 5));
  const s2 = 30 * Math.min(1, minCut / (nonExitCap / 10));
  ...
  return {
    ...
    factors: {
      ...
      connectivity: minCut / (nonExitCap / 10),
...
Object.assign(window, {
  adjacency, edgeWeight, propagateSmoke, dijkstra,
  reconstructPath, planEvacuation, computeFireSpread, safetyScore,
});
