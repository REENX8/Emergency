...
def get_token(client: httpx.Client) -> str:
    ...
    reg = client.post(f"{API}/auth/register", json={"email": DEMO_EMAIL, "password": DEMO_PASSWORD})
    ...
    login = client.post(f"{API}/auth/login", json={"email": DEMO_EMAIL, "password": DEMO_PASSWORD})
    login.raise_for_status()
    return login.json()["access_token"]
...
def main():
    ...
    with httpx.Client(timeout=30) as client:
        ...
        if sim.status_code == 200:
            ...
            if best:
                print(
                    f"  best route → {best['exit']} in {best['cost_seconds']}s via {' → '.join(best['path'])}"
                )
        else:
