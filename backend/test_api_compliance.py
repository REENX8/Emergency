...
def _seed_building(client, admin_headers, has_sprinkler=False, stair_width=1.5):
    ...
    for nk, ntype in [("r1", "room"), ("c1", "corridor"), ("exit1", "exit")]:
        client.post(
            f"/buildings/{bid}/nodes", json={"node_key": nk, "type": ntype}, headers=admin_headers
        )
    client.post(
...
def test_compliance_sprinkler_relaxes_travel_distance(client, admin_headers):
    ...
    for nk, ntype in [("r1", "room"), ("exit1", "exit")]:
        client.post(
            f"/buildings/{bid_no}/nodes",
            json={"node_key": nk, "type": ntype},
            headers=admin_headers,
        )
    client.post(
        f"/buildings/{bid_no}/edges",
    ...
    for nk, ntype in [("r1", "room"), ("exit1", "exit")]:
        client.post(
            f"/buildings/{bid_s}/nodes", json={"node_key": nk, "type": ntype}, headers=admin_headers
        )
    client.post(
        f"/buildings/{bid_s}/edges",
        json={"u_key": "r1", "v_key": "exit1", "distance_m": 45, "width_m": 2},
        headers=admin_headers,
    )
...
def test_patch_building_updates_metadata(client, admin_headers):
    ...
    r = client.patch(
        f"/buildings/{bid}", json={"has_sprinkler": True, "total_floors": 5}, headers=admin_headers
    )
    assert r.status_code == 200
    body = r.json()
